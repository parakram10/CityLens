...
def cluster_to_issue(cluster: dict[str, Any], idx: int, wards: dict[str, Any]) -> dict[str, Any]:
...
(function () {{
  ...
  window.CITYLENS_LIVE = {{ count: LIVE_ISSUES.length }};
}})();
"""
...
def main() -> None:
    ...
    live_issues = [cluster_to_issue(c, n + 1, data["wards"]) for n, c in enumerate(clusters)]
    ...
    Path(args.out).write_text(render_live_js(live_issues, keep_ids, replay_ids, src_name))
    ...
    Path(args.out).with_suffix(".json").write_text(
        json.dumps({"rev": rev, "partial": partial, "issues": live_issues, "replay": replay_ids})
    )
